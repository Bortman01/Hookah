...
1. Правите файлы в `site/`
2. Если меняли `assets/site.css` или `assets/site.js`, из корня репозитория запускаете
   `python scripts/bust_cache.py`. Скрипт допишет к ссылкам на них хеш (`site.css?v=…`).
   Без этого телефон, открывавший сайт в последние 10 минут, может взять новую страницу
   со старыми стилями из кеша
3. Коммитите в `main`
4. Из корня репозитория выкладываете папку в `gh-pages`:
...
- Карта и маршрут ведут на карточку Google «Nuahule Smoke» (place_id `ChIJ7ZgDi6uFZ0ARDMnWfTGJ0kk`,
  41.62292, 41.5934384). Yandex Go открывается сразу с этой точкой назначения. У Bolt нет публичной
  ссылки с адресом: кнопка копирует адрес в буфер и открывает приложение, гость вставляет адрес сам.
